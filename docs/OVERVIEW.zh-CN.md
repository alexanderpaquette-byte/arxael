...
> 各部分如何组合在一起的简化地图。本文只讲结构。
>
> 🌐 English version: [OVERVIEW.md](OVERVIEW.md)

**一句话：** 让许多受信任的本地 AI 智能体（agent）在**同一个项目**上、通过运行在你自己机器上的**同一个常驻、
有限流、共享的执行器**协作——它们各自开分支 → 测试 → 提交 PR，Arxael 则把**通过测试的改动落地到始终保持绿色的
`main`，并自动回滚失败的改动**；机器会**自动调整规模**，在重并发负载下**排队而非卡死**。优势在于**密度**（单台机器
在崩溃前能撑住多少智能体），而非单次构建的速度。
...
- 🟣 **合并编排器（Merge orchestrator）**（`dev.arxael.merge`，接口 `/merge/{register,submit,status}`）。
  ...
  一旦出错就自动回滚（即时、不连锁）；低负载 / 大闭包 → **批量先门禁后落地**，从不*有意*弄坏 `main`，并在某批变红时归因到罪魁 PR。
...
> 分支门禁 + 模块级验证带来**正确性**（`main` 从不*有意*破坏）。
...
flowchart TB
    ...
    ascan -->|绿| done1["保持落地 ✓<br/>即时落地"]:::good
    ascan -->|红| revert["只回滚这一个 PR<br/>（不连锁）"]:::bad

    batch --> bgate{"对整批门禁"}:::gate
    bgate -->|绿| done2["整批落地 ✓<br/>绝不落地红色合并"]:::good
...
- **分支门禁 = 正确性，乐观落地 = 低延迟**——二者合一：即时落地，`main` 从不*有意*破坏（唯一残留——未声明的跨模块耦合——记录于 [LIMITATIONS.md](LIMITATIONS.md)）。
